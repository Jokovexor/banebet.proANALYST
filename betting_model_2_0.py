...
from scipy.special import gamma as scipy_gamma  # NegativeGammaTable + ComplexGammaLayer
...
# =============================================================================
# BANEBET MATHEMATICAL LAYERS v1.0
# Komponenty matematyczne BANEBET zintegrowane z silnikiem.
# =============================================================================

# -----------------------------------------------------------------------------
# 1. CubicRationalLayer — ((B+δ+γ)/(B+γ))³
#    Przekształca sygnał bazowy przez sześcienną funkcję wymierną.
#    Używane jako FunctionTransformer w preprocessingu.
# -----------------------------------------------------------------------------

def cubic_rational_transform(X: np.ndarray,
                               B: float = 1.0,
                               gamma: float = 1.0) -> np.ndarray:
    """
    Dla każdej cechy delta w X: ((B + delta + gamma) / (B + gamma))^3
    B, gamma: parametry regulacyjne (domyślnie 1.0)
    """
    return ((B + X + gamma) / (B + gamma)) ** 3

CubicRationalLayer = FunctionTransformer(
    func=cubic_rational_transform,
    kw_args={"B": 1.0, "gamma": 1.0},
    validate=True,
)


# -----------------------------------------------------------------------------
# 2. DiscriminantSwitch — delta > 0 → 2 ścieżki predykcji
#    Warunkowe rozgałęzienie: gdy delta > 0, dwa rozwiązania (pierwiastki);
#    gdy delta <= 0, jedno rozwiązanie (wartość zegarowa/bazowa).
# -----------------------------------------------------------------------------

def discriminant_switch(delta: float,
                        p_base: float,
                        B: float = 1.0,
                        gamma: float = 1.0) -> Tuple[float, Optional[float]]:
    """
    Implementacja warunku dyskryminantu BANEBET.
    delta > 0  → dwa rozwiązania: (solution_plus, solution_minus)
    delta <= 0 → jedno rozwiązanie: (p_base, None)

    Args:
        delta:   wartość delta sygnału
        p_base:  bazowe prawdopodobieństwo z modelu
        B, gamma: parametry transformacji

    Returns:
        Krotka (sol1, sol2) — sol2 = None gdy delta <= 0
    """
    if delta > 0:
...
# -----------------------------------------------------------------------------
# 3. NegativeGammaTable — Γ(x) dla x = -0.5, -1.5, -2.5, ...
#    Tabela wartości funkcji Gamma dla ujemnych półcałkowitych.
#    Używana jako korekta nieliniowa w modelu MEBN.
# -----------------------------------------------------------------------------

NEGATIVE_GAMMA_TABLE: Dict[float, float] = {
    x: float(scipy_gamma(x))
    for x in [-0.5, -1.5, -2.5, -3.5, -4.5, -5.5, -6.5, -7.5, -8.5, -9.5]
}

def get_negative_gamma(x: float) -> float:
    """
    Zwraca Γ(x) dla x będącego ujemną połówką.
    Jeśli x nie jest w tabeli, oblicza dynamicznie.
    """
    if x in NEGATIVE_GAMMA_TABLE:
        return NEGATIVE_GAMMA_TABLE[x]
    # Dla wartości spoza tabeli — oblicz bezpośrednio
    try:
        return float(scipy_gamma(x))
    except Exception:
        return 1.0  # fallback dla osobliwości


# -----------------------------------------------------------------------------
# 4. ComplexGammaLayer — Γ(z) dla z = x + iy (liczby zespolone)
#    SciPy obsługuje liczby zespolone natywnie.
#    Zwraca moduł |Γ(z)| jako wagę wzmacniającą sygnał.
# -----------------------------------------------------------------------------

def complex_gamma_weight(x: float, y: float) -> float:
    """
    Oblicza |Γ(x + iy)| — moduł gamma zespolonego.
    Używane do wzmocnienia sygnału w przestrzeni zespolonej.
    ...
    Returns:
        Moduł |Γ(z)|, przycięty do [0.01, 10.0]
    """
    z = complex(x, y)
    try:
        result = abs(scipy_gamma(z))
        return float(np.clip(result, 0.01, 10.0))
    except Exception:
        return 1.0


# -----------------------------------------------------------------------------
# 5. OctaveRatioLayer — (σ·γ) / (σ·δ)
#    Stosunek iloczynów Cayley-oktawowych (aproksymacja przez np.dot).
#    Używane jako FunctionTransformer w preprocessingu.
# -----------------------------------------------------------------------------

def octave_ratio_transform(X: np.ndarray,
                            sigma: Optional[np.ndarray] = None) -> np.ndarray:
    """
    Dla każdego wiersza x w X:
        numerator   = dot(sigma, gamma_vec)   gdzie gamma_vec = x[n//2:]
        denominator = dot(sigma, delta_vec)   gdzie delta_vec = x[:n//2]
    Zwraca stosunek (numerator / denominator), clip [0.01, 100].

    Jeśli sigma=None, używa jedynkowego wektora (równe wagi).
    """
    ...
    results = []
    rows = X if X.ndim > 1 else X.reshape(1, -1)
    for row in rows:
        gamma_vec = row[half:half + len(sigma)]
        delta_vec = row[:len(sigma)]
        num   = float(np.dot(sigma, gamma_vec)) + 1e-9
        denom = float(np.dot(sigma, delta_vec)) + 1e-9
        results.append(np.clip(num / denom, 0.01, 100.0))
    return np.array(results).reshape(-1, 1) if len(results) > 1 else np.array(results)

OctaveRatioLayer = FunctionTransformer(
    func=octave_ratio_transform,
    validate=False,
)


# -----------------------------------------------------------------------------
# 6. NegBinomGammaHead — Negatywny rozkład dwumianowy z parametrem gamma
#    Aproksymacja przez korektę Poissona z ujemną gammą.
#    Nie wymaga TensorFlow Probability — czyste scipy/numpy.
# -----------------------------------------------------------------------------

def negbinom_gamma_pmf(k: int, mu: float, gamma_neg: float = -0.5) -> float:
    """
    Aproksymuje P(X=k) przez Poissona z korektą ujemnej gamma:
        P(X=k) ≈ Poisson(k; mu) * |Γ(gamma_neg)| / normalizacja

    Używane do modelowania liczby goli/punktów z nieliniową korektą.

    Args:
        k:         liczba zdarzeń (gole, punkty)
        mu:        oczekiwana liczba zdarzeń
        gamma_neg: parametr ujemnej gamma (domyślnie -0.5)
    """
    from scipy.stats import poisson
    lam = max(mu, 0.01)
    base_pmf = poisson.pmf(k, lam)
    correction = abs(get_negative_gamma(gamma_neg))
    # Normalizacja korekty (abs gamma_neg może być duże)
...
# -----------------------------------------------------------------------------
# 7. LogicPredicateLayer — AND / OR / TRUE / FALSE operacje na predykatach
#    Warunkowe bramki logiczne na sygnałach z modeli ensemble.
# -----------------------------------------------------------------------------

def logic_predicate(signals: Dict[str, bool],
                     mode: str = "AND") -> bool:
    """
    Ewaluuje predykat logiczny na zbiorze sygnałów.

    Args:
        signals: słownik {nazwa_modelu: bool}
        mode:    "AND" → wszystkie True | "OR" → przynajmniej jeden True
                 "TRUE" → zawsze True | "FALSE" → zawsze False
    ...
    if mode == "OR":
        return any(values)
    raise ValueError(f"Nieznany tryb predykatu: {mode}. Użyj: AND, OR, TRUE, FALSE")

LogicPredicateLayer = FunctionTransformer(
...
# -----------------------------------------------------------------------------
# 8. IdentityMorphismLayer — f: a → b (odwzorowanie tożsamościowe)
#    Czyste przejście — zachowuje wymiary, przydatne jako placeholder
#    lub do debugowania pipeline'u.
# -----------------------------------------------------------------------------

def identity_morphism(X: np.ndarray) -> np.ndarray:
    """Odwzorowanie tożsamościowe f: a → a. Nie zmienia danych."""
    return X

IdentityMorphismLayer = FunctionTransformer(
    func=identity_morphism,
    validate=True,
)


# -----------------------------------------------------------------------------
# 9. FullComplexPipeline — kompletny pipeline wszystkich 9 warstw
#    Sekwencyjne połączenie transformacji w scikit-learn Pipeline.
#    Można stosować jako preprocessing przed XGBoost/MEBN.
# -----------------------------------------------------------------------------

def build_banebet_pipeline(use_cubic: bool = True,
                            use_octave: bool = True) -> Pipeline:
    """
    Buduje pełny pipeline BANEBET z wybranych warstw transformacji.

    Kolejność:
    1. IdentityMorphismLayer  — passthrough (walidacja wejścia)
    2. CubicRationalLayer     — transformacja (B+δ+γ)³ / (B+γ)³  [opcjonalna]
    3. OctaveRatioLayer       — stosunek oktawowy σγ/σδ           [opcjonalna]

    Args:
        use_cubic:  czy zastosować CubicRationalLayer
        use_octave: czy zastosować OctaveRatioLayer (wymaga parzystej liczby cech)

    Returns:
        sklearn.pipeline.Pipeline gotowy do .fit_transform()
    """
    steps = [("identity", IdentityMorphismLayer)]
    if use_cubic:
        steps.append(("cubic_rational", CubicRationalLayer))
    if use_octave:
        steps.append(("octave_ratio", OctaveRatioLayer))
    return Pipeline(steps)


# =============================================================================
# BANEBET DECISION ENGINE v5.4 — Threshold Ladder + Fractions Matrix
# =============================================================================

def evaluate_banebet_v5_4_decision(model_predictions: List[float],
                                    true_label: Optional[float] = None) -> Dict:
    """
    Silnik decyzyjny BANEBET v5.4 — integracja z UniversalBettingEngine.

    Przyjmuje predykcje z 7 modeli ensemble i stosuje drabinkę progów
    (Threshold Ladder) oraz macierz frakcji (Fractions Matrix) do podjęcia
    decyzji BET / NO_BET.

    Args:
        model_predictions: lista prawdopodobieństw (float) z 7 modeli
        true_label:        opcjonalnie — do celów kalibracji/backtestu

    Returns:
        słownik z decyzją, pewnością i metadanymi
    """
    predictions = np.array(model_predictions)
    mean_p      = float(np.mean(predictions))
    n_models    = len(predictions)

    # Błąd = model nie popiera kierunku (p < 0.5)
    errors = int(sum(1 for p in predictions if p < 0.5))

    # --- Drabinka progów (Threshold Ladder) ---
    threshold_ladder = {
        0: 0.75,
        1: 0.50,
        2: 0.33,
    ...
    # --- Macierz frakcji (Fractions Matrix) ---
    fraction_requirements = {
        ...
        (4, 6): 0.567,
        (5, 6): 0.733,
        # 7-modelowe (pełny ensemble BANEBET)
        (1, 7): 0.300,
        (2, 7): 0.400,
        (3, 7): 0.350,
        (4, 7): 0.520,
    ...
    # A. Główny próg dla liczby błędów
    base_threshold = threshold_ladder.get(errors, 1.0)
    if mean_p < base_threshold:
        return {
            "action":   "NO_BET",
            "reason":   f"Mean P ({mean_p:.3f}) poniżej progu dla {errors} błędów ({base_threshold:.3f})",
            "mean_p":   round(mean_p, 4),
            "errors":   errors,
            "n_models": n_models,
        }

    # B. Sprawdzenie frakcji
    ...
    if fraction_key in fraction_requirements:
        ...
        if mean_p < required_p:
            return {
                "action":   "NO_BET",
                "reason":   f"Wymóg frakcyjny nie spełniony dla klucza {fraction_key} (wymagane {required_p:.3f})",
                "mean_p":   round(mean_p, 4),
                "errors":   errors,
                "n_models": n_models,
            }

    # C. Korekta przez DiscriminantSwitch — oblicz delta jako rozrzut predykcji
    delta = float(np.var(predictions))
    sol_plus, sol_minus = discriminant_switch(delta, mean_p)

    # D. Autokorekta przez NegativeGammaTable — wzmocnienie sygnału
    gamma_correction = abs(get_negative_gamma(-0.5))  # Γ(-0.5) ≈ -3.545
...
class UniversalBettingEngine:
    ...
    def predict(self, match_params: np.ndarray) -> Dict:
        ...
        # BANEBET v5.4: decyzja przez Threshold Ladder + Fractions Matrix
        # Budujemy listę predykcji na podstawie dostępnych modeli ensemble.
        # Jeśli drzewo jest wytrenowane, zbieramy p_win z każdego modelu;
        # w przeciwnym razie duplikujemy p_win (fallback dla nieprzetrenowanego stanu).
        ensemble_preds = self._collect_ensemble_predictions(match_params, p_win)
        banebet_decision = evaluate_banebet_v5_4_decision(ensemble_preds)
        results["banebet_v54"] = banebet_decision

        return results

    def _collect_ensemble_predictions(self,
                                       match_params: np.ndarray,
                                       p_win_mebn: float) -> List[float]:
        """
        Zbiera predykcje ze wszystkich aktywnych modeli ensemble w formie listy.
        MEBN/TT jest zawsze pierwszym elementem listy.

        Returns:
            Lista prawdopodobieństw (float) — po jednej na model.
        """
        preds = [p_win_mebn]  # MEBN / Tensor Train

        if not self.tree_layer.trained:
            # Jeśli model nie jest wytrenowany, duplikuj p_win jako fallback
            return preds * 7  # 7 modeli

        X = match_params.reshape(1, -1)

        # XGBoost
        try:
            p_xgb = float(np.clip(self.tree_layer.xgb_model.predict(X)[0], 0, 1))
            preds.append(p_xgb)
        except Exception:
            pass

        # RF (residual corrector — dodajemy bezpośrednią predykcję RF)
        try:
            xgb_base = float(np.clip(self.tree_layer.xgb_model.predict(X)[0], 0, 1))
            rf_res   = float(self.tree_layer.rf_model.predict(X)[0])
            p_rf     = float(np.clip(xgb_base + rf_res, 0, 1))
            preds.append(p_rf)
        except Exception:
            pass

        # KTBoost
        if KTB_AVAILABLE and self.tree_layer.ktb_model is not None:
            try:
                p_ktb = float(np.clip(self.tree_layer.ktb_model.predict(X)[0], 0, 1))
                preds.append(p_ktb)
            except Exception:
                pass

        # NGBoost
        if NGB_AVAILABLE and self.tree_layer.ngb_model is not None:
            try:
                p_ngb = float(np.clip(self.tree_layer.ngb_model.predict(X)[0], 0, 1))
                preds.append(p_ngb)
            except Exception:
                pass

        # GPBoost
        if GPB_AVAILABLE and getattr(self.tree_layer, "gpb_model", None) is not None:
            try:
                gpb_pred = self.tree_layer.gpb_model.predict(
                    data=X, gp_coords_pred=X, predict_var=False
                )
                if isinstance(gpb_pred, dict):
                    gpb_pred = gpb_pred.get("response_mean", list(gpb_pred.values())[0])
                preds.append(float(np.clip(gpb_pred[0], 0, 1)))
            except Exception:
                pass

        # Upewnij się, że lista ma przynajmniej 2 elementy (wymagane przez Fractions Matrix)
        while len(preds) < 2:
    ...
    def describe(self) -> None:
        ...
        if self.tree_layer.trained:
            ...
            for k, v in self.tree_layer.ensemble_weights.items():
                print(f"    {k:<6} = {v:.3f}")
        print(f"\n  BANEBET MATH LAYERS v1.0:")
        print(f"    CubicRationalLayer:    ✓  ((B+δ+γ)/(B+γ))³")
        print(f"    DiscriminantSwitch:    ✓  delta>0 → 2 rozwiązania")
        print(f"    NegativeGammaTable:    ✓  Γ(x) dla x=-0.5,-1.5,... ({len(NEGATIVE_GAMMA_TABLE)} wpisów)")
        print(f"    ComplexGammaLayer:     ✓  |Γ(x+iy)| via scipy")
        print(f"    OctaveRatioLayer:      ✓  (σ·γ)/(σ·δ)")
        print(f"    NegBinomGammaHead:     ✓  Poisson + korekta Γ(ujemna)")
        print(f"    LogicPredicateLayer:   ✓  AND/OR/TRUE/FALSE")
        print(f"    IdentityMorphismLayer: ✓  f: a → a")
        print(f"    FullComplexPipeline:   ✓  build_banebet_pipeline()")
        print(f"  BANEBET DECISION v5.4:   ✓  Threshold Ladder + Fractions Matrix")
        print(f"{'='*60}\n")
